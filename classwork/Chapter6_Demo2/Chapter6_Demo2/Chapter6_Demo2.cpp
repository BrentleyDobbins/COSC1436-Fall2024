...
//    + you can overload any function needed
...
int ReadInt(std::string message)
{
    ...
    return value; //to return the value from the caller
...
int ReadInt(std::string message, int minValue)
{
    ...
    do
    {
        ...
        if (value < minValue)
            std::cout << "Value must at least " << minValue << std::endl;
...
// atof() ::= takes a string to a float
// atoi() ::= takes a string to an ineger